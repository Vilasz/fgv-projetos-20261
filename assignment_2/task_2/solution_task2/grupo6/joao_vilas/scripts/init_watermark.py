...
def run_init(settings: Settings) -> int:
    if not settings.watermark_sql_path.exists():
        logger.error("SQL de inicialização não encontrado: %s", settings.watermark_sql_path)
        return 1

    sql_text = settings.watermark_sql_path.read_text(encoding="utf-8")
    conn = None
    ...
    try:
        ...
        with conn.cursor() as cursor:
            logger.info("Aplicando %s", settings.watermark_sql_path.name)
