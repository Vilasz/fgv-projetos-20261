...
def assert_non_empty(df, table_name: str) -> None:
    ...
    if total <= 0:
        raise RuntimeError(f"[VALIDATION] {table_name} esta vazia")
...
def assert_no_missing_fk(fact_df, dim_df, fact_key: str, dim_key: str, description: str) -> None:
    ...
    if missing != 0:
        raise RuntimeError(
            f"[VALIDATION] FK invalida em {description}: {missing} chave(s) sem dimensao"
        )
...
productlines = read_mysql_table("productlines")  # noqa: F841 - reservado p/ enriquecimentos futuros
...
print("[VALIDATION] Iniciando validacoes objetivas")
